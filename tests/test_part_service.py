...
def import_parts_from_excel(file_storage, user, config):
    ...
    for row in reader:
        ...
        # --- ИЗМЕНЕНИЕ: Внедрен более надежный способ извлечения данных ---
        ...
        part_id = get_cell_value("Обозначение")
        name = get_cell_value("Наименование")
        ...
        material = get_cell_value("Прим.", "Не указан")
        if not material:
            material = "Не указан"

        operations_str = get_cell_value("Операции")
        ...
        new_part = Part(
            ...
            name=name,
            quantity_total=int(get_cell_value("Кол-во", "1") or 1),
            size=get_cell_value("Размер"),
            material=material,
